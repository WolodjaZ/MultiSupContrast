#!/bin/bash
#SBATCH --job-name=mutlisupcontrast_decoder
...
EXPERIMENT_PATH="/home/wolodja5/results/experiments_decoder"
...
srun --output=${EXPERIMENT_PATH}/%j.out --error=${EXPERIMENT_PATH}/%j.err --label singularity run --nv pytorch.sif \
    python -m  torch.distributed.launch --nproc_per_node=1 tmp/MultiSupContrast/main_decoder.py \
    --data=$DATASET_PATH \
    --data-name=CELEBA \
